...
public final class DtsReader implements ElementaryStreamReader {
    ...
    public DtsReader(String str, int i, int i2, String str2) {
    ...
    private void updateFormatWithDtsHeaderInfo(DtsUtil.DtsHeader dtsHeader) {
        ...
        Format build = (format == null ? new Format.Builder() : format.buildUpon()).setId(this.formatId).setContainerMimeType(this.containerMimeType).setSampleMimeType(dtsHeader.mimeType).setChannelCount(dtsHeader.channelCount).setSampleRate(dtsHeader.sampleRate).setLanguage(this.language).setRoleFlags(this.roleFlags).build();
        this.format = build;
